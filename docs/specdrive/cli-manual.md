...
다만 현재 문서 작업의 권장 리듬은  
자동 실행보다 **명시적 history 저장 + 정규화 프롬프트 출력 + 대화형 보강** 쪽으로 옮겨가는 중이다.
...
4. 필요 시 `doc reinforce -Execute` 또는 사람이 복사한 prompt 로 Codex 대화 진행
5. 사람이 문서 반영
6. `doc history-save -Source codex-reinforce`
7. 3~6 반복
...
- 역할:
  ...
  - `session start`, `session save` 는 콘솔 출력만 제공
...
powershell -ExecutionPolicy Bypass -File specdrive/scripts/doc/history-save.ps1 -Target board-overview -Source codex-reinforce
```
...
현재 `confirm` 은 문서 적용 또는 별도 검토 체크리스트가 필요할 때 쓰는 후속 명령으로 본다.  
즉 현재 권장 루프의 중심은 `draft-save -> reinforce-prompt -> history-save` 다.

현재 자동 실행까지 포함한 문서 반영 흐름은 보통 아래처럼 이해한다.
...
```powershell
powershell -ExecutionPolicy Bypass -File specdrive/scripts/doc/draft-save.ps1 -Target board-overview -Note "요구사항 문장 재정리 전 개발자 초안 저장"
```

현재 `draft-save` 는 preview 생성보다  
현재 문서를 바로 history snapshot 으로 남기는 명령이다.

### 7.2 reinforce-prompt

```powershell
powershell -ExecutionPolicy Bypass -File specdrive/scripts/doc/reinforce-prompt.ps1 -Target board-overview
```

interactive 모드 예시는 다음과 같다.

```powershell
powershell -ExecutionPolicy Bypass -File specdrive/scripts/doc/reinforce-prompt.ps1 -Target board-overview -Mode interactive
```

이번 반복의 구체적인 의도를 같이 넘기려면:

```powershell
powershell -ExecutionPolicy Bypass -File specdrive/scripts/doc/reinforce-prompt.ps1 -Target board-overview -Mode direct -Focus requirements -Note "이번에는 요구사항 누락만 보강하고 구조 재설계는 하지 말 것"
```

현재 `reinforce-prompt` 는 Codex 대화 시작용 정규화 앵커 프롬프트를 출력한다.  
특히 interactive 모드는 Codex와 대화하면서 방향을 맞춰가는 경우를 전제로 한다.
...
```powershell
powershell -ExecutionPolicy Bypass -File specdrive/scripts/doc/history-save.ps1 -Target board-overview -Source codex-reinforce -DryRun
```

preview 만 보고 싶으면:

```powershell
powershell -ExecutionPolicy Bypass -File specdrive/scripts/doc/history-save.ps1 -Target board-overview -Source codex-reinforce -PreviewOnly
```

현재 `history-save` 는 기본 경로에서  
현재 적용 문서를 바로 history snapshot 과 note 로 저장한다.

`-Source` 로 아래 같은 출처 구분을 남길 수 있다.

- `developer-draft`
- `codex-reinforce`
- `manual-update`

현재 저장 시 함께 처리하는 내용은 다음과 같다.
...
powershell -ExecutionPolicy Bypass -File specdrive/specdrive.ps1 doc history-save -Target board-overview -Source codex-reinforce
```

실제 Codex 실행까지 보려면 다음처럼 `doc reinforce -Execute` 를 붙인다.
...
- `session status` 는 읽기 전용 상태 확인 명령으로 보고, 문서 수정이나 저장 흐름으로 바로 이어지지 않게 둔다.
...
- 특히 `session start` copy prompt 는 현재 상태를 먼저 요약하고, 개발자 요청 전에는 파일을 직접 수정하지 말라는 경계를 포함한다.
- 특히 `session save` copy prompt 는 `docs/AI_CONTEXT.md` 반영 초안을 먼저 보여주고, 개발자 승인 전에는 파일을 직접 수정하지 말라는 경계를 포함한다.
...
현재 `session save` 의 권장 사용 순서는 아래와 같다.

1. `specdrive/specdrive.ps1 session save`
2. 출력된 copy prompt 를 Codex 대화에 붙여넣기
3. Codex 가 제안한 `docs/AI_CONTEXT.md` 반영 초안 검토
4. 저장이 필요하면 개발자가 "저장해줘" 같이 명시적으로 요청
5. 그 시점에만 실제 `docs/AI_CONTEXT.md` 수정 진행

현재 `session start` 의 권장 사용 순서는 아래와 같다.

1. `specdrive/specdrive.ps1 session start`
2. 출력된 copy prompt 를 Codex 대화에 붙여넣기
3. Codex 가 관련 문서를 직접 읽고 현재 상태를 복구
4. Codex 가 현재 focus, 다음 진입점, 주의할 변경 범위를 먼저 정리
5. 그 다음 실제 문서 작업이나 구현 작업으로 들어가기

현재 `session status` 의 권장 사용 순서는 아래와 같다.

1. `specdrive/specdrive.ps1 session status`
2. `docs/AI_CONTEXT.md` 기준 현재 상태 요약 확인
3. 지금 세션이 어디까지 정리되어 있는지 빠르게 판단
4. 필요하면 그 다음 `session start` 또는 실제 `doc` 작업으로 이동
...
2. `doc reinforce-prompt` 의 note / focus / 대화형 시작 경험 보강
3. `docs/projects/board/specs/02-requirements.md` 에 동일 흐름 반복 적용
