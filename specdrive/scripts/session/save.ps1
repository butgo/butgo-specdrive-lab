...
Write-Host "이번 세션을 저장하려고 해."
Write-Host "현재 브랜치는 '$branchLabel' 이야."
Write-Host "현재 Git 상태 요약은 '$statusSummary' 이고, 변경 영역 요약은 '$areaSummary' 이야."
Write-Host "변경 파일 샘플은 아래와 같아."
foreach ($line in $sampleBlock) {
    Write-Host $line
}
if ($hasMorePaths) {
    Write-Host "- ... and $remainingPathCount more path(s). 필요하면 git status로 상세 목록을 확인해줘."
}
Write-Host "이 변경 요약과 샘플을 기준으로 session save용 요약 초안을 작성해줘."
Write-Host "긴 대화 전체를 다시 요약하려 하지 말고, 변경 파일과 현재 상태 문서 기준으로 필요한 최소 문맥만 사용해줘."
Write-Host "반드시 이번 세션에서 한 일, 검증한 것, docs/AI_CONTEXT.md 반영 후보, 다음 진입점, 보류 사항을 구분해줘."
Write-Host "직접 파일을 수정하지 말고, 사람이 검토해서 반영할 수 있는 초안으로만 제안해줘."
