...
$draft = Get-SpecdriveCommitMessageDraft -ChangedPaths $changedPaths

Write-Host "[git git-message] current branch:"
Write-Host "  $(if ([string]::IsNullOrWhiteSpace($branchName)) { 'unknown' } else { $branchName })"
Write-Host "[git git-message] change summary:"
Write-Host "  - $($changeOverview.Summary)"
Write-Host "  - changed areas: $($changeOverview.AreaSummary)"
Write-Host "[git git-message] changed path sample:"
...
if ($Detailed) {
    Write-Host "[git git-message] changed paths detail:"
...
Write-Host "[git git-message] subject:"
Write-Host "  $($draft.Subject)"
Write-Host "[git git-message] body:"
foreach ($line in $draft.Body) {
    Write-Host "  $line"
}

if ($DryRun) {
    Write-Host ""
    Write-Host "[git git-message] dry-run only. No one-time artifact generated."
    exit 0
}
Write-Host ""
Write-Host "[git git-message] no one-time artifact saved."
