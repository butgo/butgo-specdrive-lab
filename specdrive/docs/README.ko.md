# specdrive/docs/README.ko.md

[English](README.md) | [한국어](README.ko.md)

## 1. 문서 목적

이 문서는 `specdrive/docs/**` 영역의 **대표 진입 문서**다.
...
세부 작업 규칙은 `specdrive/docs/AGENTS.md`,  
현재 상태 복구는 `docs/AI_CONTEXT.md`,  
세부 목록은 `specdrive/docs/index.md` 와 개별 문서를 따른다.
...
### 5.2 plan 단계
확정 또는 준확정된 개발 문서를 실행 가능한 작업 구조로 분해하는 단계다.
...
### 5.3 dev 단계
승인된 plan 결과를 기준으로 실제 코딩과 테스트를 진행하는 단계다.
...
- 승인된 roadmap에서 현재 실행할 Work Package를 선택한다.
- `work-index.md` 에 현재 실행 포인터를 설정한다.
- 현재 Work Package 안에서 코딩을 진행한다.
- 현재 Work Package 기준으로 테스트 또는 검증을 실행한다.
- 실행 결과를 `work-log.md` 에 요약하고 후속 동기화 후보를 정리한다.
...
현재 `$session restore` 는 VSCode 또는 Codex 재시작 뒤 `docs/AI_CONTEXT.md` 와 현재 Git 작업트리 상태를 함께 확인해 이어갈 위치를 복구하는 읽기 전용 action 으로 보는 편이 맞다.
...
SpecDrive의 Work System은 후보 작업을 수집하고, Phase / Cycle / Work Package / Task 단위로 분해한 뒤, 현재 작업 포인터를 기준으로 AI와 사람이 함께 실행하고 동기화하는 작업 운영 체계다.

자세한 기준은 다음 문서를 따른다.

- `work-system.md`

프로젝트별 적용 문서는 각 프로젝트의 `work/` 디렉터리에 둔다.
...
- Work Package 추출과 plan 분해 절차
- Phase / Cycle / Work Package / Task 관리 방향
- 현재 Work Package 기준 dev 실행 흐름
...
### 우선순위 2
- `doc` / `plan` / `dev` 단계 구분을 문서와 명령 차원에서 고정
