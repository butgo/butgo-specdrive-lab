...
Apply Draft와 승인용 copy-ready prompt는 사용자가 `apply`를 요청했거나 반영 프롬프트를 요청한 경우에만 출력한다.

history snapshot/note는 기본 `generate` 출력에 포함하지 않는다.  
history snapshot/note는 apply 승인 후 선택 항목 또는 후속 action 후보로만 다룬다.
...
- `plan extract-candidates apply`: 승인된 후보 초안을 `work-candidates.md`에 반영하기 위한 변경안을 출력하는 흐름이며, 자동 수정은 아니다.
...
후속작업 프롬프트 출력 기준은 wizard 동작이 애매할 때 `specdrive/rules/skill-wizard-rule.md` 를 따른다.
...
Approval Prompt:
- 사용자가 `apply` 또는 반영 프롬프트를 요청한 경우에만 출력
```
...
프로젝트 `work/` 문서를 생성하거나 갱신하는 apply는 사용자가 명시적으로 요청한 경우에만 반영안을 출력한다.
history snapshot/note는 apply 승인 후 선택 항목 또는 후속 action 후보로만 다룬다.
