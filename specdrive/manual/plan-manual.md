...
work-tasks.md
  - Task
  ↓
work-roadmap.md 또는 work-packages.md
  - WP
  ↓
dev impl-run
  ↓
...
```text
Candidate -> Phase -> Cycle -> Task Candidate -> Work Package

Task Candidate = CAND 후보에서 파생된 해야 할 일의 최소 후보
Work Package = Task 후보를 묶은 AI 실행 단위
Task 후보 추출 action = $plan task-split
WP 패키징 action = $plan wp
```

표현은 항상 `Candidate -> Phase -> Cycle -> Task Candidate -> Work Package` 방향으로 쓴다.  
Task는 WP 구성을 위한 재료이며, WP는 AI에게 맡길 수 있는 실행 단위다.

Taskmaster류 도구를 참고할 때도 SpecDrive 용어를 우선한다.

```text
외부 Task ~= SpecDrive Work Package
외부 Subtask ~= SpecDrive Task Candidate
```

외부 도구의 task/subtask 구조를 그대로 가져오지 않고, SpecDrive에서는 `Candidate -> Phase -> Cycle -> Task Candidate -> Work Package` 흐름으로 해석한다.  
...
기존 파일이 있으면 자동 반영하지 않고 `Next Step`에 후속 선택지를 다음처럼 나눈다.

- `overwrite`: 이번 Plan Update Candidate를 기준으로 기존 파일 덮어쓰기용 copy-ready prompt를 출력한다.
- `keep`: 기존 파일 유지용 copy-ready prompt를 출력한다.
- `revise`: `$plan extract-candidates revise <option number or revision request>` 안내만 출력한다.

overwrite와 keep prompt는 선택 가능한 prompt 후보이며 현재 action 안에서 실행하지 않는다.
revise는 별도 `$plan extract-candidates revise` 실행으로 다룬다.
...
이 경우 Plan Update Candidate에 blocking issue가 없으면 `Copy-ready Prompt`에 파일 생성 prompt를 하나 출력한다.
...
- `generate`: 기준 문서를 읽고 현재 action의 Plan Update Candidate 초안을 새로 만들고, 마지막에 Revise Options를 출력한다.
- `revise`: 기존 Plan Update Candidate를 대상으로 사용자가 선택한 옵션 번호 또는 직접 입력한 수정 요건을 반영하는 흐름이다.
...
`revise`는 AI가 혼자 다시 검토하는 단계가 아니라, 기존 Plan Update Candidate를 대상으로 사용자의 선택 또는 수정 요건을 반영하는 단계다.
사용자가 옵션 번호를 선택한 경우 해당 옵션의 의도에 따라 후보 목록을 재정리한다.
사용자가 직접 수정 요건을 입력한 경우 그 요건을 우선한다.
수정 요청이 없으면 긴 질문을 하지 않고 Revise Options만 다시 출력한다.
수정 요청이 충분히 분명한 경우에만 같은 plan action 안에서 같은 계층의 revised Plan Update Candidate를 만든다.
`revise` 역시 문서 확정, history 저장, dev 전환을 수행하지 않는다.

Revise Options는 다음 기준으로 출력한다.
...
- `plan extract-candidates revise`: 사용자가 선택한 Revise Options 번호 또는 직접 수정 요건을 기준으로 revised Plan Update Candidate를 만드는 흐름이다.

즉 `doc-work draft`는 **개발자 초안 보존**이고, `plan extract-candidates generate`는 **문서 기반 후보 생성**이다.
`plan extract-candidates revise`는 **plan 후보 초안 수정 흐름**이며, 문서 반영이나 history 저장이 아니다.
...
`generate`의 `Next Step`에는 다음 안내를 짧게 포함한다.

- 후보가 적절하면 파일 반영 요청 또는 `$plan phase-split`을 진행할 수 있다.
- 후보 수정이 필요하면 `$plan extract-candidates revise`와 함께 Revise Options 번호 또는 직접 수정 요건을 입력한다.
- 범위가 잘못되었으면 Source Scope 또는 Scope Note를 바꿔 다시 generate한다.
- `work-candidates.md`가 없고 생성이 명확한 다음 단계이면 `Copy-ready Prompt`에 파일 생성 prompt를 하나 출력한다.
- `work-candidates.md`가 이미 있으면 overwrite와 keep은 copy-ready prompt로, revise는 안내로 출력한다.
...
현재 plan 문서의 CAND 후보를 WP 구성을 위한 Task 후보로 나눈다.

Task 후보 기준:

- 해야 할 일의 최소 후보여야 한다.
- CAND 후보에서 파생된 작업 항목이어야 한다.
...
- WP 구성을 위한 재료로 사용할 수 있어야 한다.
- 아직 WP 소속, 실행 순서, Codex 실행 단위를 확정하지 않는다.
...
- 이 단계는 최종 구현 Task를 확정하지 않는다.
- 이 단계는 Work Package를 확정하지 않는다.
- 현재 실행 포인터를 설정하지 않는다.
...
- AI에게 맡길 수 있는 실행 단위여야 한다.
- 관련 Task 후보들을 묶은 작업 패키지여야 한다.
- scope, target files, reference docs, verification 기준이 있어야 한다.
- Codex/Gemini/Ollama/Antigravity Runner가 사용할 수 있는 단위여야 한다.
...
| `$plan task-split` | `docs/projects/{project}/work/work-tasks.md` | CAND와 연결된 Task 후보, 예상 산출물, 검증 기준 후보 | 아직 WP 소속 또는 실행 순서 확정 아님 |
| `$plan wp` | `docs/projects/{project}/work/work-roadmap.md` | Task 후보를 묶은 Work Package 후보, scope, target files, reference docs, verification | 아직 dev 실행 확정 아님 |
...
| 파일 | 이유 |
|---|---|
| `docs/projects/{project}/work/work-index.md` | 현재 실행 포인터는 dev 책임 |
...
2. `work-roadmap.md`
3. `work-tasks.md`
4. 필요한 경우 `work-policy.md`
5. 필요한 경우 `work-log.md`

`work-index.md` 는 dev 단계에서 생성 또는 갱신하는 것이 원칙이다.
...
$plan task-split
  - CAND 후보 전체를 WP 구성을 위한 Task 후보로 나눈다.

$plan wp
  - Task 후보를 게시글 등록/조회, 수정/삭제, 검증 보강 같은 AI 실행 패키지로 묶는다.
```

이후 개발자가 plan 결과를 승인하면 `$dev impl-run` 에서 선택된 Work Package를 기준으로 구현을 진행한다.
